...
/**
 *  @brief This function prepares the command of WNM_PS_CFG
 *
 *  @param pmpriv       A pointer to mlan_private structure
 *  @param cmd          A pointer to HostCmd_DS_COMMAND structure
 *  @param pdata_buf    A pointer to data buffer
 ...
 */
mlan_status
wlan_cmd_wnm_ps_cfg(IN pmlan_private pmpriv,
		    OUT HostCmd_DS_COMMAND *cmd,
		    mlan_ioctl_req *pioctl_req, IN t_void *pdata_buf)
{
	mlan_ds_pm_cfg *pm_cfg = (mlan_ds_pm_cfg *)pdata_buf;
	mlan_ds_wnm_ps_cfg_data *pcfg_data =
		(mlan_ds_wnm_ps_cfg_data *) & pm_cfg->param.wnm_ps_cfg_data;
	HostCmd_DS_WNM_PS_CFG *pcmd_wnm_ps_cfg =
		(HostCmd_DS_WNM_PS_CFG *) & cmd->params.wnm_ps_cfg;
	t_u16 tlv_buf_left = 0;
	MrvlIEtypesHeader_t *tlv = MNULL;
	mlan_ds_wnm_ps_bss_max_idle_tlv_t *bss_max_tlv = MNULL;
	mlan_ds_wnm_ps_dms_tlv_t *dms_tlv = MNULL;
	tclas_element_tlv_t *tclas_tlv = MNULL;
	t_u8 *pos = MNULL;
	t_u16 travel_len = 0;

	ENTER();

	cmd->command = wlan_cpu_to_le16(HostCmd_CMD_WNM_PS_CFG);
	cmd->size = sizeof(HostCmd_DS_WNM_PS_CFG) + S_DS_GEN;
	cmd->result = 0;

	tlv_buf_left = pcfg_data->tlv_data_len;
	pos = pcmd_wnm_ps_cfg->wnm_ps_cfg_cmd_data;
	tlv = (MrvlIEtypesHeader_t *)pcfg_data->wnm_ps_tlv;
	...
	while (tlv_buf_left >= sizeof(MrvlIEtypesHeader_t)) {
		if (tlv_buf_left < (sizeof(MrvlIEtypesHeader_t) + tlv->len))
			break;
		switch (tlv->type) {
		case TLV_TYPE_BSS_MAX_IDLE:
			memcpy(pmpriv->adapter, pos + travel_len, (t_u8 *)tlv,
			       sizeof(MrvlIEtypesHeader_t) + tlv->len);
			bss_max_tlv =
				(mlan_ds_wnm_ps_bss_max_idle_tlv_t *) (pos +
								       travel_len);
			bss_max_tlv->type = wlan_cpu_to_le16(bss_max_tlv->type);
			bss_max_tlv->len = wlan_cpu_to_le16(bss_max_tlv->len);
			bss_max_tlv->bss_max_idle_cfg.bss_max_idle_action =
				wlan_cpu_to_le16(bss_max_tlv->bss_max_idle_cfg.
						 bss_max_idle_action);
		...
		case TLV_TYPE_DMS:
			memcpy(pmpriv->adapter, pos + travel_len, (t_u8 *)tlv,
			       sizeof(MrvlIEtypesHeader_t) + tlv->len);
			dms_tlv =
				(mlan_ds_wnm_ps_dms_tlv_t *) (pos + travel_len);
			dms_tlv->type = wlan_cpu_to_le16(dms_tlv->type);
			dms_tlv->len = wlan_cpu_to_le16(dms_tlv->len);
			dms_tlv->dms_cfg.dms_action =
				wlan_cpu_to_le16(dms_tlv->dms_cfg.dms_action);
			dms_tlv->dms_req.dms_req_type =
				wlan_cpu_to_le16(dms_tlv->dms_req.dms_req_type);
			dms_tlv->dms_req.dms_desc_elements =
				wlan_cpu_to_le16(dms_tlv->dms_req.
						 dms_desc_elements);
			dms_tlv->dms_req.dms_total_data_len =
				wlan_cpu_to_le16(dms_tlv->dms_req.
						 dms_total_data_len);
			/* handle sub TLVs for DMS */
			{
				t_u16 sub_tlv_buf_left = 0;
				t_u16 sub_travel_len = 0;
				MrvlIEtypesHeader_t *sub_tlv = MNULL;
				t_u8 *sub_pos = MNULL;

				sub_tlv_buf_left =
					dms_tlv->dms_req.dms_total_data_len;
				sub_pos = dms_tlv->dms_req.dms_desc_tlv;
				sub_tlv =
					(MrvlIEtypesHeader_t *)dms_tlv->dms_req.
					dms_desc_tlv;
			   ...
			   IN t_u16 cmd_action, IN t_void *pdata_buf)
{
	...
	cmd->size =
		wlan_cpu_to_le16((sizeof(HostCmd_DS_FW_AUTO_RECONNECT)) +
				 S_DS_GEN);
			 ...
			 IN t_void *pdata_buf, IN t_void *pcmd_buf)
{
	...
	case HostCmd_CMD_802_11_FW_WAKE_METHOD:
		ret = wlan_cmd_802_11_fw_wakeup_method(pmpriv, cmd_ptr,
						       cmd_action, pdata_buf);
		break;
	case HostCmd_CMD_WNM_PS_CFG:
		ret = wlan_cmd_wnm_ps_cfg(pmpriv, cmd_ptr,
