...
/**
 *  @brief This function handles the command response of WNM_PS_CFG
 *
 *  @param pmpriv       A pointer to mlan_private structure
 *  @param resp         A pointer to HostCmd_DS_COMMAND
 *  @param pioctl_buf   A pointer to mlan_ioctl_req structure
 ...
 */
mlan_status
wlan_ret_wnm_ps_cfg(IN pmlan_private pmpriv,
		    const IN HostCmd_DS_COMMAND *resp,
		    OUT mlan_ioctl_req *pioctl_buf)
{
	mlan_ds_pm_cfg *pm_cfg = MNULL;
	mlan_ds_wnm_ps_cfg_data *pcfg_data = MNULL;
	const HostCmd_DS_WNM_PS_CFG *presp_wnm_ps_cfg =
		&resp->params.wnm_ps_cfg;
	...
	if (pioctl_buf) {
		...
		pcfg_data =
			(mlan_ds_wnm_ps_cfg_data *) & pm_cfg->param.
			wnm_ps_cfg_data;
		tlv_buf_left = wlan_le16_to_cpu(resp->size)
			- sizeof(HostCmd_DS_GEN)
			- sizeof(HostCmd_DS_WNM_PS_CFG);

		tlv = (MrvlIEtypesHeader_t *)presp_wnm_ps_cfg->
			wnm_ps_cfg_cmd_data;
		pos = pcfg_data->wnm_ps_tlv;

	/** Endianess conversion of tlvs */
		while (tlv_buf_left >= sizeof(MrvlIEtypesHeader_t)) {
			...
			case TLV_TYPE_BSS_MAX_IDLE:
				bss_max_tlv =
					(mlan_ds_wnm_ps_bss_max_idle_tlv_t *)
					tlv;
				bss_max_tlv->type = tlv_type;
				bss_max_tlv->len = tlv_len;
				bss_max_tlv->bss_max_idle_cfg.
					bss_max_idle_action =
					wlan_le16_to_cpu(bss_max_tlv->
							 bss_max_idle_cfg.
							 bss_max_idle_action);
				...
				break;
			case TLV_TYPE_DMS:
				dms_tlv = (mlan_ds_wnm_ps_dms_tlv_t *) tlv;
				dms_tlv->type = tlv_type;
				dms_tlv->len = tlv_len;
				dms_tlv->dms_cfg.dms_action =
					wlan_le16_to_cpu(dms_tlv->dms_cfg.
							 dms_action);
				dms_tlv->dms_req.dms_req_type =
					wlan_le16_to_cpu(dms_tlv->dms_req.
							 dms_req_type);
				...
				/* handle sub TLVs for DMS */
				{
					...
					sub_tlv_buf_left =
						...
						dms_total_data_len;
					sub_tlv =
						(MrvlIEtypesHeader_t *)dms_tlv->
						dms_req.dms_desc_tlv;
					sub_pos =
						pos + travel_len +
						sizeof(MrvlIEtypesHeader_t)
						+ sizeof(dms_tlv->dms_cfg)
						+ sizeof(dms_tlv->dms_req);
		...
		pcfg_data->tlv_data_len = travel_len;
	}
			     ...
			     IN t_void *pcmd_buf, IN t_void *pioctl)
{
	...
	case HostCmd_CMD_WNM_PS_CFG:
		ret = wlan_ret_wnm_ps_cfg(pmpriv, resp, pioctl_buf);
		break;
